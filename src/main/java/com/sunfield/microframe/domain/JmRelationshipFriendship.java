...
public class JmRelationshipFriendship extends BaseDomain{
	...
	@ApiModelProperty(value="好友类型，0为正常好友，2为请求中好友（主方请求时添加值为2记录，对方同意后将你为主方的记录设置为0，并添加值为0的自身为主方的记录，如果拒绝，将你为主方的记录设置为3），3为已拒绝好友。查询好友时按主方字段user_id查值为0或1的记录，查询非好友请求状态时按主方字段user_id查值为2或3的记录。无记录者为不显示任何状态的非好友。", dataType="int")
	...
	private String userName = "";//默认空串，用于缓存键
